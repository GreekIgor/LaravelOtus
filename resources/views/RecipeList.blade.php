...
<div class="container py-5">
    ...
    <div class="row">
        <div class="col-lg-3">
            <form action="{{ route('recipes.list') }}" method="GET" id="filter-form">
                <div class="card p-3 sticky-sidebar shadow-sm border-0">
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">Можно указать несколько через запятую</small>
                    </div>
                    ...
                    <a href="{{ route('recipes.list') }}" class="btn btn-outline-secondary btn-sm w-100 mt-2">
                        <i class="bi bi-x-circle me-2"></i>Сбросить всё
                    </a>
                    
                    @if(request()->hasAny(['name', 'ingredients', 'difficulty', 'time']))
                        <div class="mt-3">
                            <small class="text-muted">
                                <i class="bi bi-info-circle me-1"></i>
                                Активных фильтров: {{ collect([request('name'), request('ingredients'), request('difficulty'), request('time')])->filter()->count() }}
                            </small>
                        </div>
                    @endif
                </div>
        ...
        <div class="col-lg-9">
            @if(request()->hasAny(['name', 'ingredients', 'difficulty', 'time']))
                <div class="alert alert-info mb-4">
                    ...
                    @if(request('name'))
                        <span class="badge bg-primary me-1">Название: {{ request('name') }}</span>
                    @endif
                    @if(request('ingredients'))
                        <span class="badge bg-primary me-1">Ингредиенты: {{ request('ingredients') }}</span>
                    @endif
                    @if(request('difficulty'))
                        <span class="badge bg-primary me-1">Сложность: {{ implode(', ', request('difficulty')) }}</span>
                    @endif
                    @if(request('time'))
                        <span class="badge bg-primary me-1">Время: {{ request('time') }}</span>
                    @endif
                    <a href="{{ route('recipes.list') }}" class="btn btn-sm btn-outline-secondary ms-2">
            ...
            @if($recipes->total() > 0)
                <div class="mb-3 text-muted">
                    <small>Найдено рецептов: <strong>{{ $recipes->total() }}</strong></small>
                </div>
            @endif
            ...
            <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                ...
                <div class="col">
                    <div class="card recipe-card shadow-sm border-0">
                        @can('update', $recipe)
                            <a href="{{ route('recipe-edit', ['recipe' => $recipe->id]) }}" class="edit-badge" title="Редактировать">
                        ...
                        <div class="card-body">
                            ...
                            <a href="{{ route('recipe.detail', ['recipe' => $recipe->id]) }}" class="btn btn-sm btn-outline-primary mt-auto w-100">Открыть рецепт</a>
                        </div>
                    </div>
                </div>
                @empty
